...
public class UserSourceRealm extends AuthorizingRealm {
  ...
  public boolean isPermittedAll(PrincipalCollection subjectPrincipal, Collection<Permission> requiredPermissions) {
    ...
    Collection<Permission> subjectPermissions = info.getObjectPermissions();
  ...
  private boolean implies(Collection<Permission> subjectPermissions, Permission requiredPermission) {
  ...
  // does one of the subject permissions imply the required permission
  private void checkImplies(Collection<Permission> subjectPermissions, Permission requiredPermission) {
    for (Permission subjectPermission : subjectPermissions) {
      if (subjectPermission instanceof ShiroPermission) {
        if (((ShiroPermission) subjectPermission).checkImplies(requiredPermission)) {
          return;
        }
      }
    }
